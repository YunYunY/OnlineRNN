...
def test_mnistpermute():
    ...
    batch = next(iter(d.dataloader))
    data = batch[0][:opt.batch_size]
    permute_idx = torch.Tensor(np.random.permutation(28).astype(np.float64)).long()
    # permute_idx = torch.Tensor(np.asarray([x for x in range(11, 28)] + [x for x in range(11)]).astype(np.float64)).long()
